...
Let $J$ be the $2n\times 2n$-matrix whose only non-zero entries lie along its anti-diagonal, in positions $(i,2n{+}1{-}i)$.
The entry is 1 if $i\leq n$ and $-1$ for $i>n$.
Then, if $u,v\in V$, we have $\langle u,v\rangle = u^T J v$.

With respect to these coordinates, two both the standard coordinate flag $\Fdot'$ and the reverse coordinate flag $\Fdot$
are isotropic.
...
The Schubert cell $\Omega^\circ_\emptyset \Fdot$ is dense in $\LG(n)$ ans elements of it are represented as the column span of a matrix
$\left(\begin{smallmatrix}I_n\\X\end{smallmatrix}\right)$, where $X$ is symmetric across its anti-diagonal.\newpage
