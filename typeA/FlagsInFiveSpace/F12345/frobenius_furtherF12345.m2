...
inputfile = get "furtherF12345data.txt";
filelist = lines(inputfile);
problems = {};
for file in filelist do(
	problems = append(problems, value(file)));
f = "further_frobenius_outputF12345.txt" << "";
for problem in problems do(
	f << problem << endl;
	if problem#1 >= 7 then(
	    f << frobeniusAlgorithm(problem#0,10009,problem#1,1000) << endl << endl);
	if problem#1 == 3 then(
	    f << frobeniusDegreeThree(problem#0,10009,problem#1,1000) << endl << endl);
	if problem#1 == 4 then(
	    f << frobeniusDegreeFour(problem#0,10009,problem#1,1000) << endl << endl);
	if problem#1 == 5 then(
	    f << frobeniusDegreeFive(problem#0,10009,problem#1,1000) << endl << endl);
	if problem#1 == 6 then(
	    f << frobeniusDegreeSix(problem#0,10009,problem#1,1000) << endl << endl);
	quit;
	);
