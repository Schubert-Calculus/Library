...
completePermutation(List,ZZ):=(w,n) ->(
	...
	-- If w is already complete, then just return w. (I commented out the line below. Didn't actually check if the permutation was complete).
...
getEquations(Matrix,List,List):=(H,conditions,flagType) ->(
  ...
  n:=last(flagType);
	...
	for c in conditions do(
		...
		if isCondition(c,flagType)==false then error(toString(c#0)|" is not a condition on the flag "| toString(flagType));
	),
	launderedConditions:={};
	for c in conditions do(
