...
-- Computes the P(alpha)(F^{-1}) matrix that is essential in finding a minimal number of generators for the ideal of a Schubert problem.
...
-- Computes a polynomial ring in n variables and the ideal inside generated by the elementary symmetric polynomials in that many variables.
...
-- Computes the intersection number of a Schubert problem using cohomology in the full Type A flag manifold.
intA = method()
...
-- Computes the intersection number of a Schubert problem using cohomology in a general Type A flag manifold.
partialIntA = method()
